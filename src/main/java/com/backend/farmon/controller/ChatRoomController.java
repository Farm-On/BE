...
public class ChatRoomController {
    ...
    @Parameters({
            ...
            @Parameter(name = "read", description = "읽음 여부 필터링. 안 읽은 채팅방만 필터링 시에는 false, 이외에는 true 입니다.", required = true),
            @Parameter(name = "page", description = "페이지 번호, 1부터 시작입니다.", required = true)
    })
    ...
    // 전문가 이름과 일치하는 전체 채팅 목록 조회
    @Operation(
            summary = "전문가 이름으로 검색하여 일치하는 전문가와의 전체 채팅 목록 조회 API",
            description = "전문가 이름을 검색하여 일치하는 전체 채팅 목록을 조회하는 API이며, 페이징을 포함합니다. " +
    ...
    @Parameters({
            @Parameter(name = "userId", description = "로그인한 유저의 아이디(pk)", required = true),
            @Parameter(name = "read", description = "읽음 여부 필터링. 안 읽은 채팅방만 필터링 시에는 false, 이외에는 true 입니다.", required = true),
            @Parameter(name = "expertName", description = "검색할 전문가 이름입니다.", required = true),
            @Parameter(name = "page", description = "페이지 번호, 1부터 시작입니다.", required = true)
    })
    ...
    // 작물 이름으로 검색하여 일치하는 견적과 관련된 채팅 목록 조회
    @Operation(
            summary = "작물 이름으로 검색하여 일치하는 견적과 관련된 채팅 목록 조회",
            description = "작물 이름으로 검색하여 일치하는 견적과 관련된 채팅 목록을 조회하는 API이며 페이징을 포함합니다. " +
                    "유저 아이디, 읽음 여부 필터, 검색할 작물 이름, 페이지 번호를 쿼리 스트링으로 입력해주세요."
    )
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200",description = "OK, 성공")
    })
    @Parameters({
            @Parameter(name = "userId", description = "로그인한 유저의 아이디(pk)", required = true),
            @Parameter(name = "read", description = "읽음 여부 필터링. 안 읽은 채팅방만 필터링 시에는 false, 이외에는 true 입니다., required = true"),
            @Parameter(name = "cropsName", description = "검색할 작물 이름입니다.", required = true),
            @Parameter(name = "page", description = "페이지 번호, 1부터 시작입니다.", required = true)
    })
    ...
    @Parameters({
            @Parameter(name = "userId", description = "로그인한 유저의 아이디(pk)", required = true),
            @Parameter(name = "chatRoomId", description = "입장하려는 채팅방의 아이디", required = true),
    })
    @GetMapping("/room")
    ...
    @Parameters({
            @Parameter(name = "userId", description = "로그인한 유저의 아이디(pk)"),
            @Parameter(name = "chatRoomId", description = "삭제하려는 채팅방의 아이디"),
    })
    @DeleteMapping("/room")
